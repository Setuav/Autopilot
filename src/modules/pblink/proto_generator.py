#!/usr/bin/env python3
"""
uORB to Protobuf Generator

Generates .proto files and C++ converter code from uORB message definitions.
Supports both uplink (uORB -> Proto) and downlink (Proto -> uORB) conversions.
"""
import os
import argparse
import re
...
def uorb_to_proto_type(uorb_type):
    """Jinja2 filter to convert uORB types to Protobuf types."""
    type_map = {
        'uint64': 'uint64',
        'uint32': 'uint32',
        'uint16': 'uint32',
...
def parse_msg_file(msg_path):
    """Parses a .msg file and returns a list of fields."""
    fields = []
...
def find_msg_file(msg_dir, topic_name_camel):
    """Find .msg file in standard PX4 locations."""
    possible_paths = [
        os.path.join(msg_dir, f"{topic_name_camel}.msg"),
        os.path.join(msg_dir, "versioned", f"{topic_name_camel}.msg")
    ]
...
def list_topic_names(config):
    return [topic['name'] for topic in config.get('topics', []) if 'name' in topic]

def process_topic(topic, msg_dir, proto_template, package_name, proto_dir, yaml_file_path):
    """Process a single topic and return topic data dict."""
    ...
    if 'msg_file' in topic:
        ...
        if not msg_file_path:
            print(f"Warning: Explicit msg file '{explicit_filename}' not found in standard locations. Skipping '{topic_name_snake}'.")
            return None
    else:
        msg_file_path = find_msg_file(msg_dir, topic_name_camel)

        if not msg_file_path:
            print(f"Warning: Could not find .msg file for '{topic_name_snake}'. Skipping.")
            return None
...
def main():
    parser = argparse.ArgumentParser(description="Generate Protobuf and C++ converter files from uORB messages.")
    ...
    if args.list_topics:
        print(";".join(list_topic_names(config)))
        return

    required_args = {
        '--msg-dir': args.msg_dir,
        '--template-dir': args.template_dir,
        '--proto-dir': args.proto_dir,
        '--generated-dir': args.generated_dir,
    }
    missing_args = [name for name, value in required_args.items() if not value]

    if missing_args:
        parser.error(f"missing required arguments for generation: {', '.join(missing_args)}")
    ...
    package_name = config.get('package', 'px4_protos')
    ...
    for topic in config.get('topics', []):
        ...
        if direction == 'uplink':
            ...
            print(f"Warning: Unknown direction '{direction}' for topic '{topic.get('name')}'. Skipping.")
    ...
    if all_topics:
        ...
        print(f"Generated {h_output_path}")
        ...
        print(f"Generated {topics_h_output_path}")
        ...
        print(f"Generated {downlink_h_output_path}")
        ...
        print(f"Generated {uplink_h_output_path}")

    print(f"\nGenerated {len(uplink_topics)} uplink topics, {len(downlink_topics)} downlink topics")
